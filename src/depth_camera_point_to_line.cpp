...
int main (int argc, char* argv[])
{
    ...
    pass_cloud_in.setFilterLimits(0.0, argv[3]); // 设置过滤的范围，这里过滤掉 z 坐标大于 1.0 的点
    ...
    *cloud_tr = *cloud_icp;// 备份cloud_icp赋值给cloud_tr为后期使用
    ...
    ne.setKSearch (30);// 指定临近点数量

    ne.setInputCloud(cloud_icp);
    //ne.setRadiusSearch(0.05); // 设置法向量估计的半径
    ne.compute(*source_normals);
    pcl::copyPointCloud (*cloud_icp, *source_normals);

    ne.setInputCloud(cloud_in);
    //ne.setRadiusSearch(0.05); // 设置法向量估计的半径
    ne.compute(*target_normals);
    pcl::copyPointCloud (*cloud_in, *target_normals);
    ...
    icp.setTransformationEpsilon(1e-8);     // 变换阈值
    icp.setMaximumIterations(iterations);           // 最大迭代次数

    // 运行配准算法
    icp.align(*source_normals);
    ...
    pcl::copyPointCloud (*source_normals, *cloud_icp);
    pcl::copyPointCloud (*target_normals, *cloud_in);
    // 可视化ICP的过程与结果
